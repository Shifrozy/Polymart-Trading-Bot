...
class LiveTrader:
    ...
    async def run(self):
        """Main trading loop"""
        self.running = True
        logger.info(f"Starting live trader (Paper Mode: {self.paper_trade})")
    ...
    async def _check_entry(self, current_time, window_start, window_end, prices):
        ...
        if signal.signal:
            ...
            if self.paper_trade:
                logger.info(f"[PAPER] Entered {signal.signal} on {signal.asset} @ {signal.laggard_price}")
            else:
    ...
    async def _check_exit(self, current_time, prices):
        ...
        if current_price is None:
            logger.warning(f"Missing price for {self.current_position.asset}")
            return
    ...
    async def _close_position(self, exit_time, exit_price, reason):
        ...
        logger.info(f"CLOSED: {self.current_position.side} {self.current_position.asset} | "
                   f"Entry: {self.current_position.entry_price:.3f} Exit: {exit_price:.3f} | "
                   f"P&L: ${pnl_usd:.2f} | {reason}")
        
        if self.paper_trade:
            logger.info(f"[PAPER] Exited position")
        else:
    ...
    async def _settle_position(self, reason):
        ...
        if self.current_position:
            # Get final settlement price (assume 1.0 for win, 0.0 for loss)
            # In real implementation, fetch actual settlement from Polymarket
    ...
    async def _execute_order(self, action, asset, side, amount):
        """Execute real order on Polymarket (implement with actual API)"""
